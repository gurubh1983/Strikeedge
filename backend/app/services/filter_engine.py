...
class FilterEngine:
    ...
    # Frontend operator -> Backend operator
    OPERATOR_ALIASES = {
        '>': 'gt', '>=': 'gte', '<': 'lt', '<=': 'lte', '==': 'eq', '=': 'eq', '!=': 'neq', '≠': 'neq',
    }
    ...
    def _normalize_params(self, indicator_code: str, params: dict) -> dict:
        ...
        resolved = self._resolve_indicator(str(indicator_code))
    ...
    def _resolve_operator(self, op: str) -> str:
        """Resolve frontend operator to backend operator."""
        return self.OPERATOR_ALIASES.get(op, op)
    ...
    def calculate(self, indicator_code: str, df: pd.DataFrame, params: dict, options_data: dict = None) -> float:
        ...
        normalized = self._normalize_params(indicator_code, params or {})
    ...
    def evaluate_condition(self, df: pd.DataFrame, condition: dict, options_data: dict = None) -> bool:
        ...
        try:
            ...
            operator = self._resolve_operator(str(condition.get('operator', 'gt')))
    ...
    def _get_value(self, df: pd.DataFrame, value_spec: dict, options_data: dict = None) -> float:
        ...
        if value_spec.get('type') == 'number':
            return float(value_spec.get('value', 0))
        if value_spec.get('type') == 'indicator':
        ...
        raise ValueError(f"Unknown value type: {value_spec.get('type')}")
    ...
    # ============================================
    # ADX, SUPERTREND, VOLATILITY, VOLUME
    # ============================================
    ...
    def _prev_day_high(self, df: pd.DataFrame, params: dict) -> float:
        return df['high'].iloc[-2] if len(df) >= 2 else np.nan
    
    def _prev_day_low(self, df: pd.DataFrame, params: dict) -> float:
        return df['low'].iloc[-2] if len(df) >= 2 else np.nan
    
    def _prev_day_close(self, df: pd.DataFrame, params: dict) -> float:
        return df['close'].iloc[-2] if len(df) >= 2 else np.nan
    
    def _day_change_pct(self, df: pd.DataFrame, params: dict) -> float:
        return ((df['close'].iloc[-1] - df['close'].iloc[-2]) / df['close'].iloc[-2]) * 100 if len(df) >= 2 else 0.0
    
    def _gap_pct(self, df: pd.DataFrame, params: dict) -> float:
        return ((df['open'].iloc[-1] - df['close'].iloc[-2]) / df['close'].iloc[-2]) * 100 if len(df) >= 2 else 0.0
    ...
    def _calculate_indicator_series(self, indicator_code: str, df: pd.DataFrame, params: dict) -> pd.Series:
        resolved = self._resolve_indicator(indicator_code)
        norm = self._normalize_params(indicator_code, params)
        if resolved == 'RSI':
            period = norm.get('period', 14)
            if HAS_PANDAS_TA:
            ...
            return 100 - (100 / (1 + rs))
        if resolved == 'RSI_SMA':
            rsi_period = norm.get('rsi_period', 14)
            sma_period = norm.get('sma_period', 14)
            if HAS_PANDAS_TA:
                rsi_s = ta.rsi(df['close'], length=rsi_period)
                if rsi_s is None:
                    return pd.Series([np.nan] * len(df))
                return rsi_s.rolling(window=sma_period).mean()
            delta = df['close'].diff()
            gain = delta.where(delta > 0, 0).rolling(window=rsi_period).mean()
            loss = (-delta.where(delta < 0, 0)).rolling(window=rsi_period).mean()
            rs = gain / loss
            rsi_s = 100 - (100 / (1 + rs))
            return rsi_s.rolling(window=sma_period).mean()
        if resolved in ('SMA', 'EMA'):
            period = norm.get('period', 20)
            if resolved == 'SMA':
